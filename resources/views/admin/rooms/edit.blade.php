...
<div class="mb-10">
    ...
    <h1 class="text-4xl font-black text-gray-900 tracking-tight">Chỉnh sửa phòng: {{ $room->room_number }}</h1>
    <p class="text-gray-500 mt-2 text-sm font-medium">Cập nhật thông tin chi tiết cho phòng hiện tại trong hệ thống.</p>
</div>

<div class="max-w-3xl bg-white p-10 rounded-[3rem] shadow-xl shadow-gray-100/50 border border-gray-50">
    <form action="{{ route('rooms.update', $room) }}" method="POST" class="space-y-10">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div class="col-span-2 md:col-span-1">
                ...
                <input type="text" name="room_number" value="{{ $room->room_number }}" required class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm transition font-black text-gray-800">
            </div>

            <div class="col-span-2 md:col-span-1">
                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4 pl-1">Giá thuê (VNĐ/tháng)</label>
                <input type="number" name="price" value="{{ (int)$room->price }}" required class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm transition font-black text-gray-800">
            </div>

            <div class="col-span-2 md:col-span-1">
                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4 pl-1">Diện tích (m²)</label>
                <input type="number" step="0.1" name="area" value="{{ $room->area }}" class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm transition font-black text-gray-800">
            </div>

            <div class="col-span-2 md:col-span-1">
                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4 pl-1">Số người tối đa</label>
                <input type="number" name="max_people" value="{{ $room->max_people }}" class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm transition font-black text-gray-800">
            </div>

            <div class="col-span-2 md:col-span-1">
                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4 pl-1">Trạng thái phòng</label>
                <select name="status" class="w-full px-6 py-5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm transition appearance-none font-black text-gray-800">
                    <option value="empty" {{ $room->status == 'empty' ? 'selected' : '' }}>Còn trống</option>
